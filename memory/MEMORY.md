...
- **2026-04-02** — GRO-43 PR #38: confirmed branch matches `origin/main` (no rebase conflicts; merge into main fast-forwards); updated PR description with **Why this PR matters** (Paperclip close path, versioned cron contract, no auto-repair change) and merge-conflict note.
- **2026-04-02** — GRO-43 PR #38: rebased `cursor/GRO-43-4640` onto `origin/main` and force-pushed; updated PR body with rebase note, operator `sync-cron-jobs.sh` reminder, and verification commands (Linear linkback stays via bot).
- **2026-04-02** — GRO-43 workflow health: Paperclip’s public API has no terminal `failed` status, so `cron-paperclip-lifecycle.sh` mapping `error` → `failed` caused PATCH failures and left run issues stuck open (`todo`/`in_progress`). Error runs now close as `done` with the existing `error` finish comment (and `CRON_ERROR_DETAILS` comment). Restored committed `cron/jobs.json` (aligned to `tests/fixtures/core-cron-jobs.json`; `delivery.to` is a `-100…` placeholder in git — `./tools/sync-cron-jobs.sh` merges the real group id from `~/.openclaw/cron/jobs.json`) so `runtime_cron_matches` and CI have the four core jobs. Stopped gitignoring `cron/jobs.json`. Docs: `AGENTS.md`, `docs/agent-tasks.md`, `NorthStar.md`. Test: `test_committed_cron_jobs_match_core_fixture`, updated lifecycle error test.
