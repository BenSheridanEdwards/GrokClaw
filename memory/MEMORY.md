...
- **2026-04-01** — GRO-39: Added `--status` to `tools/grokclaw-doctor.sh` — prints one line of `key=value` pairs for gateway, Paperclip, Ollama, Telegram, launchd jobs, health crontab, cron JSON validation, repo/runtime cron sync, and `openclaw cron list` auth; read-only (no heal, no Telegram); exit 1 if any check fails like default doctor.
