        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self._write_stub(
                ...
                textwrap.dedent(
                    ...
                    #!/bin/sh
                    set -eu
                    printf '%s\\n' '{"market_id":"m5","condition_id":"c5","question":"Will B happen?","odds_yes":0.98,"odds_no":0.02,"volume":18000,"selection_source":"bonding_copy","copy_strategy":{"status":"ok","consensus_probability_yes":0.98,"confidence":0.77,"traders_with_matching_positions":1}}'
                    """
                ),
            )
            self._write_stub(
            ...
            env = os.environ.copy()
            env["WORKSPACE_ROOT"] = str(workspace)
            env["ALPHA_NOW"] = "2026-04-09T12:40:00Z"
            result = subprocess.run(
                ["python3", str(self.script), str(workspace)],
                cwd=str(self.repo),
                env=env,
