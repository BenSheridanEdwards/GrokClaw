...
STRATEGY PRIORITY: Start with **bonding-copy mode** (Dexter-style): favor high-confidence copy-trader positions near resolution (evaluation window: 95-100c, with 97-99c still preferred when available) when bonding wallets show alignment and liquidity is acceptable. A single matching bonding wallet is valid; more matching wallets should raise confidence. Use a broader near-resolution window (up to ~36h) to collect more evaluation samples while staying bonding-only. If no valid bonding setup, HOLD (do not fall back to whale-copy). Avoid 15-minute latency-arb style markets as a primary strategy (fee structure can erase edge).
