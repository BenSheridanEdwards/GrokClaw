#!/usr/bin/env python3
"""
Sentient resolve: check unresolved trades against Manifold API, append results.
Paper trading only. Stdlib only. Called from sentient-resolve.sh.
"""
import json
import math
import os
...
def fetch_market(market_id):
    url = f"{API_BASE}/v0/market/{market_id}"
...
def market_is_resolved(market):
    return market.get("isResolved") is True and market.get("resolution") is not None
...
def main():
    if len(sys.argv) < 2:
        print("usage: _sentient_resolve.py <workspace_root>", file=sys.stderr)
        sys.exit(1)
    ...
    for t in trades:
        ...
        print(
            ...
            f"{'WIN' if won else 'LOSS'} (P&L: {pnl_amount:+.2f})"
        )
