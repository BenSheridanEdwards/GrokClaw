...
# Each Paperclip run gets its own ephemeral session to avoid polluting
# the main agent session used by the daily suggestion cron.
...
cd "$WORKSPACE"
exec picoclaw agent --message "$MESSAGE" --session "$SESSION_KEY"
